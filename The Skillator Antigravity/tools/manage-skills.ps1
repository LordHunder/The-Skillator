...
$CacheDir = Join-Path $env:USERPROFILE ".gemini\skills-cache"
$ProjectRoot = Split-Path -Parent $PSScriptRoot

if ($GlobalGemini) {
    $ActiveSkillsDir = Join-Path $env:USERPROFILE ".gemini\skills"
} else {
    $ActiveSkillsDir = Join-Path $ProjectRoot ".agent\skills"
}
...
if ($Install) {
    ...
    # Text adaptation for Gemini / Antigravity
    $SkillFile = Join-Path $destPath "SKILL.md"
    if (Test-Path $SkillFile) {
        $text = Get-Content $SkillFile -Raw
        $text = $text -replace "Claude Code", "Antigravity/Gemini"
        $text = $text -replace "Claude", "the AI agent"
        $text = $text -replace "Anthropic", "Antigravity"
        $text = $text -replace "claude", "agent"
        Set-Content -Path $SkillFile -Value $text -Force
    }
    
    Write-Host ("[OK] Skill '" + $targetSkill.Name + "' installed and adapted for Gemini successfully!") -ForegroundColor Green
...
# Default Help
Write-Host "================================================================================" -ForegroundColor Cyan
Write-Host "The Skillator -- Universal Multi-Repo Skill Manager (Antigravity Edition)" -ForegroundColor White
Write-Host "================================================================================" -ForegroundColor Cyan
...
Write-Host "  -GlobalGemini                                  # Target global Gemini skills folder" -ForegroundColor Gray
